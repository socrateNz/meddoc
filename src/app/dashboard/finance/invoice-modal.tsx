...
export default function InvoiceModal({ transaction, organizationName, organizationLogoUrl, open: externalOpen, onOpenChange: externalOnOpenChange, triggerBtn }: InvoiceModalProps) {
  ...
  // La référence doit être celle que le pharmacien devra saisir pour finaliser (PendingInvoice.id,
  // cf. dispensePendingInvoice), pas l'id interne de la transaction — les deux documents ont des
  // id Mongo distincts. Repli sur l'id de transaction pour les ventes sans médicament (rien à
  // dispenser côté pharmacie, la référence n'y sert alors qu'à l'affichage du ticket).
