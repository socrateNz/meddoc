...
// La caisse est une opération physique par clinique (CashRegister/CashSession scopées
// organizationId) — il n'existe pas d'équivalent "caisse agrégée toute la holding" comme pour
// Finance. Un ADMIN choisit ici la clinique dont il veut consulter la caisse.
export default async function HoldingCaissePage() {
  const activeUser = await getCurrentUser();
  if (!activeUser) redirect("/login");
  if (activeUser.organization?.type === "CLINIC") {
    redirect(`/dashboard/clinics/${activeUser.organizationId}/caisse`);
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1 animate-fade-up">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Sélectionnez une clinique pour consulter l&apos;état de ses caisses.
        </p>
      </div>
      ...
      {clinics.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {clinics.map((c) => (
            <Link
              key={c.id}
              href={`/dashboard/clinics/${c.id}/caisse`}
